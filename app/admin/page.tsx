  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        if (!silent) alert('獲取數據失敗: ' + json.error);
        if (res.status === 401) setIsAuthenticated(false);
      }
    } catch (err) {
      console.error(err);
      if (!silent) alert('獲取數據失敗，請稍後再試');
    }
    setLoading(false);
  }, []);
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center font-sans">
        <form onSubmit={handleLogin} className="bg-white p-10 rounded-2xl shadow-2xl w-96">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-black text-gray-800">Aixelis 管理後台</h2>
            <p className="text-gray-500 text-sm mt-2">請輸入管理員密碼以查看預約</p>
          </div>
